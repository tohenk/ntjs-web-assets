...
function getSaxesParser() {
    if (!SaxesParserCtor) {
        try {
            SaxesParserCtor = node_module.createRequire(require('url').pathToFileURL(__filename).href)('saxes').SaxesParser;
        }
        catch (_a) {
...
class ComponentParser {
     ...
     */
    reset() {
        this._decoder = new node_string_decoder.StringDecoder('utf8');
        this._stack = [];
...
class Component {
    ...
    _connect() {
        ...
        const socket = node_net.connect({ host, port });
        this.socket = socket;
    ...
    _sendHandshake(streamId) {
        ...
        const digest = node_crypto.createHash('sha1')
