 ...
 * the page does. It doesn't need to: RFC 7395 delivers exactly one top-level
 * stanza or nonza per websocket frame, so the engine's {@link StanzaView}
 * (tag name plus a few attributes) can be extracted from the opening tag with
 * a regex. Divergence between the page and worker environments is prevented
 * by sharing the engine, not the parser.
 */
import type { StanzaView } from './stream-management';
/**
...
//# sourceMappingURL=sm-parse.d.ts.map