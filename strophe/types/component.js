...
class Component {
    ...
    _onStreamStart(attrs) {
        ...
        if (!streamId) {
            ...
            this._conn._changeConnectStatus(Status.CONNFAIL, 'Missing stream id in component stream header');
    ...
    _handleHandshakeResult(stanza) {
        ...
        if (stanza.nodeName === 'handshake') {
            ...
            const condition = ((_a = stanza.firstElementChild) === null || _a === void 0 ? void 0 : _a.nodeName) || 'not-authorized';
            log.error(`Component handshake failed: ${condition}`);
            this._conn._changeConnectStatus(Status.AUTHFAIL, condition);
            this._conn._doDisconnect(condition);
        }
        else {
            log.error(`Unexpected stanza <${stanza.nodeName}> before component handshake completed`);
            this._conn._changeConnectStatus(Status.AUTHFAIL, 'unexpected-stanza');
            this._conn._doDisconnect();
        }
    }
    /**
    ...
    _checkStreamError(streamError) {
        ...
        log.error(`Component stream error: ${condition}${text ? ' - ' + text : ''}`);
        this._conn._changeConnectStatus(Status.CONNFAIL, condition);
        this._conn._doDisconnect(condition);
    }
    /**
    ...
    _onParseError(error) {
        ...
        this._conn._changeConnectStatus(Status.CONNFAIL, ErrorCondition.BAD_FORMAT);
        this._conn._doDisconnect(ErrorCondition.BAD_FORMAT);
    }
    /**
    ...
    _onClose() {
        if (this._conn.connected && !this._conn.disconnecting) {
            ...
            this._conn._changeConnectStatus(Status.CONNFAIL, 'The TCP connection could not be established or was disconnected.');
            this._conn._doDisconnect();
        }
        else {
