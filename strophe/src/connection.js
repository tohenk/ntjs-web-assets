...
var _builder = _interopRequireWildcard(require("./builder.js"));
...
class Connection {
  ...
  constructor(service) {
    ...
    (0, _utils.addCookies)(this.options.cookies);
    ...
    this.iqFallbackHandler = new _handler.default(
    /**
     * @param {Element} iq
     */
    iq => this.send((0, _builder.$iq)({
      type: 'error',
      id: iq.getAttribute('id')
    }).c('error', {
      'type': 'cancel'
    }).c('service-unavailable', {
      'xmlns': _core.default.NS.STANZAS
    })), null, 'iq', ['get', 'set']);

    // initialize plugins
    for (const k in _core.default._connectionPlugins) {
      if (Object.prototype.hasOwnProperty.call(_core.default._connectionPlugins, k)) {
        const F = function () {};
        F.prototype = _core.default._connectionPlugins[k];
        this[k] = new F();
        this[k].init(this);
      }
    }
  ...
  setProtocol() {
    ...
    if (this.options.worker) {
      this._proto = new _core.default.WorkerWebsocket(this);
    } else if (this.service.indexOf('ws:') === 0 || this.service.indexOf('wss:') === 0 || proto.indexOf('ws') === 0) {
      this._proto = new _core.default.Websocket(this);
    } else {
      this._proto = new _core.default.Bosh(this);
    }
  }
  ...
  connect(jid, pass, callback, wait, hold, route, authcid) {
    ...
    this.authzid = _core.default.getBareJidFromJid(this.jid);
    /** Authentication identity (User name) */
    this.authcid = authcid || _core.default.getNodeFromJid(this.jid);
    ...
    this.domain = _core.default.getDomainFromJid(this.jid);
    this._changeConnectStatus(_constants.Status.CONNECTING, null);
  ...
  attach(jid, sid, rid, callback, wait, hold, wind) {
    if (this._proto instanceof _core.default.Bosh) {
      return this._proto._attach(jid, sid, rid, callback, wait, hold, wind);
    } else {
      throw new _errors.SessionError('The "attach" method is not available for your connection protocol');
    }
  }
  ...
  restore(jid, callback, wait, hold, wind) {
    if (!(this._proto instanceof _core.default.Bosh) || !this._sessionCachingSupported()) {
      throw new _errors.SessionError('The "restore" method can only be used with a BOSH connection.');
    }
  ...
  send(stanza) {
    ...
    if (Array.isArray(stanza)) {
      stanza.forEach(s => this._queueData(s instanceof _builder.default ? s.tree() : s));
    } else {
      const el = stanza instanceof _builder.default ? stanza.tree() : stanza;
      this._queueData(el);
    }
    this._proto._send();
  }
  ...
  addTimedHandler(period, handler) {
    const thand = new _core.default.TimedHandler(period, handler);
  ...
  addHandler(handler, ns, name, type, id, from, options) {
    const hand = new _handler.default(handler, ns, name, type, id, from, options);
  ...
  registerSASLMechanisms(mechanisms) {
    this.mechanisms = {};
    mechanisms = mechanisms || [_core.default.SASLAnonymous, _core.default.SASLExternal, _core.default.SASLOAuthBearer, _core.default.SASLXOAuth2, _core.default.SASLPlain, _core.default.SASLSHA1, _core.default.SASLSHA256, _core.default.SASLSHA384, _core.default.SASLSHA512];
   ...
   */
  disconnect(reason) {
    this._changeConnectStatus(_constants.Status.DISCONNECTING, reason);
    if (reason) {
      _core.default.warn('Disconnect was called because: ' + reason);
    } else {
      _core.default.info('Disconnect was called');
    }
    if (this.connected) {
      let pres = null;
      this.disconnecting = true;
      if (this.authenticated) {
        pres = (0, _builder.$pres)({
          'xmlns': _core.default.NS.CLIENT,
          'type': 'unavailable'
        });
      }
      ...
      _core.default.warn('Disconnect was called before Strophe connected to the server');
  ...
  _changeConnectStatus(status, condition, elem) {
      ...
      if (Object.prototype.hasOwnProperty.call(_core.default._connectionPlugins, k)) {
        ...
        if (plugin.statusChanged) {
          try {
            ...
            _core.default.error(`${k} plugin caused an exception changing status: ${err}`);
          }
        }
      }
    }
    ...
    if (this.connect_callback) {
      try {
        ...
        _core.default._handleError(e);
        _core.default.error(`User connection callback caused an exception: ${e}`);
      }
    }
  }
  ...
  _doDisconnect(condition) {
    ...
    _core.default.debug('_doDisconnect was called');
    ...
    this._changeConnectStatus(_constants.Status.DISCONNECTED, condition);
    this.connected = false;
  }
   ...
   * stanza.
   * @param {Element} req - The request that has data ready.
  ...
  _dataRecv(req, raw) {
    ...
    if (this.xmlInput !== _core.default.Connection.prototype.xmlInput) {
    ...
    if (this.rawInput !== _core.default.Connection.prototype.rawInput) {
      if (raw) {
        this.rawInput(raw);
      } else {
        this.rawInput(_core.default.serialize(elem));
      }
    }
    ...
    if (type !== null && type === 'terminate') {
      ...
      if (cond !== null) {
        ...
        this._changeConnectStatus(_constants.Status.CONNFAIL, cond);
      } else {
        this._changeConnectStatus(_constants.Status.CONNFAIL, _core.default.ErrorCondition.UNKOWN_REASON);
      }
    ...
    _core.default.forEachChild(elem, null, child => {
      const matches = [];
      this.handlers = this.handlers.reduce((handlers, handler) => {
        try {
          ...
          _core.default.warn('Removing Strophe handlers due to uncaught exception: ' + e.message);
        }
        return handlers;
      }, []);
   ...
   *
   * @param {Element} req - The current request.
  ...
  _connect_cb(req, _callback, raw) {
    _core.default.debug('_connect_cb was called');
    this.connected = true;
    let bodyWrap;
    try {
      bodyWrap = this._proto._reqToData(req);
    } catch (e) {
      if (e.name !== _core.default.ErrorCondition.BAD_FORMAT) {
        throw e;
      }
      this._changeConnectStatus(_constants.Status.CONNFAIL, _core.default.ErrorCondition.BAD_FORMAT);
      this._doDisconnect(_core.default.ErrorCondition.BAD_FORMAT);
    }
    if (!bodyWrap) {
      return;
    }
    ...
    if (this.rawInput !== _core.default.Connection.prototype.rawInput) {
      if (raw) {
        ...
        this.rawInput(_core.default.serialize(bodyWrap));
      }
    }
    const conncheck = this._proto._connect_cb(bodyWrap);
    if (conncheck === _constants.Status.CONNFAIL) {
      return;
    }
    ...
    if (bodyWrap.getElementsByTagNameNS) {
      hasFeatures = bodyWrap.getElementsByTagNameNS(_core.default.NS.STREAM, 'features').length > 0;
    } else {
  ...
  _attemptSASLAuth(mechanisms) {
    ...
    for (let i = 0; i < mechanisms.length; ++i) {
      ...
      const request_auth_exchange = (0, _builder.$build)('auth', {
        'xmlns': _core.default.NS.SASL,
        'mechanism': this._sasl_mechanism.mechname
      });
      if (this._sasl_mechanism.isClientFirst) {
        const response = this._sasl_mechanism.clientChallenge(this);
        request_auth_exchange.t((0, _abab.btoa)(response));
      }
  ...
  async _sasl_challenge_cb(elem) {
    const challenge = (0, _abab.atob)((0, _utils.getText)(elem));
    const response = await this._sasl_mechanism.onChallenge(this, challenge);
    const stanza = (0, _builder.$build)('response', {
      'xmlns': _core.default.NS.SASL
    });
    if (response !== '') {
      stanza.t((0, _abab.btoa)(response));
    }
   ...
   */
  _attemptLegacyAuth() {
    if (_core.default.getNodeFromJid(this.jid) === null) {
      // we don't have a node, which is required for non-anonymous
      // client connections
      this._changeConnectStatus(_constants.Status.CONNFAIL, _core.default.ErrorCondition.MISSING_JID_NODE);
      this.disconnect(_core.default.ErrorCondition.MISSING_JID_NODE);
    } else {
      // Fall back to legacy authentication
      this._changeConnectStatus(_constants.Status.AUTHENTICATING, null);
      this._addSysHandler(this._onLegacyAuthIQResult.bind(this), null, null, null, '_auth_1');
      this.send((0, _builder.$iq)({
        'type': 'get',
        'to': this.domain,
        'id': '_auth_1'
      }).c('query', {
        xmlns: _core.default.NS.AUTH
      }).c('username', {}).t(_core.default.getNodeFromJid(this.jid)).tree());
    }
  }
  ...
  _onLegacyAuthIQResult(elem) {
    ...
    const iq = (0, _builder.$iq)({
      type: 'set',
      id: '_auth_2'
    }).c('query', {
      xmlns: _core.default.NS.AUTH
    }).c('username', {}).t(_core.default.getNodeFromJid(this.jid)).up().c('password').t(this.pass);
    if (!_core.default.getResourceFromJid(this.jid)) {
      // since the user has not supplied a resource, we pick
      // a default one here.  unlike other auth methods, the server
      // cannot do this for us.
      this.jid = _core.default.getBareJidFromJid(this.jid) + '/strophe';
    }
    iq.up().c('resource', {}).t(_core.default.getResourceFromJid(this.jid));
  ...
  _sasl_success_cb(elem) {
    if (this._sasl_data['server-signature']) {
      let serverSignature;
      const success = (0, _abab.atob)((0, _utils.getText)(elem));
    ...
    _core.default.info('SASL authentication succeeded.');
    ...
    streamfeature_handlers.push(this._addSysHandler(elem => wrapper(streamfeature_handlers, elem), _core.default.NS.STREAM, 'features', null, null));
  ...
  _onStreamFeaturesAfterSASL(elem) {
    ...
    if (!this.do_bind) {
      this._changeConnectStatus(_constants.Status.AUTHFAIL, null);
      return false;
    } else if (!this.options.explicitResourceBinding) {
      this.bind();
    } else {
      this._changeConnectStatus(_constants.Status.BINDREQUIRED, null);
    }
    return false;
  }
  ...
  bind() {
    if (!this.do_bind) {
      _core.default.log(_core.default.LogLevel.INFO, `Strophe.Connection.prototype.bind called but "do_bind" is false`);
      return;
    }
    this._addSysHandler(this._onResourceBindResultIQ.bind(this), null, null, null, '_bind_auth_2');
    const resource = _core.default.getResourceFromJid(this.jid);
    if (resource) {
      this.send((0, _builder.$iq)({
        type: 'set',
        id: '_bind_auth_2'
      }).c('bind', {
        xmlns: _core.default.NS.BIND
  ...
  _onResourceBindResultIQ(elem) {
    if (elem.getAttribute('type') === 'error') {
      _core.default.warn('Resource binding failed.');
      const conflict = elem.getElementsByTagName('conflict');
      let condition;
      if (conflict.length > 0) {
        condition = _core.default.ErrorCondition.CONFLICT;
      }
      this._changeConnectStatus(_constants.Status.AUTHFAIL, condition, elem);
      return false;
    }
    ...
    if (bind.length > 0) {
      ...
      if (jidNode.length > 0) {
        ...
        this.jid = (0, _utils.getText)(jidNode[0]);
        if (this.do_session) {
          this._establishSession();
        } else {
          this._changeConnectStatus(_constants.Status.CONNECTED, null);
        }
      }
    } else {
      _core.default.warn('Resource binding failed.');
      this._changeConnectStatus(_constants.Status.AUTHFAIL, null, elem);
      return false;
    }
  }
  ...
  _establishSession() {
    if (!this.do_session) {
      throw new Error(`Strophe.Connection.prototype._establishSession ` + `called but apparently ${_core.default.NS.SESSION} wasn't advertised by the server`);
    }
    ...
    this.send((0, _builder.$iq)({
      ...
      xmlns: _core.default.NS.SESSION
    }).tree());
  }
  ...
  _onSessionResultIQ(elem) {
    if (elem.getAttribute('type') === 'result') {
      ...
      this.authenticated = false;
      _core.default.warn('Session creation failed.');
      this._changeConnectStatus(_constants.Status.AUTHFAIL, null, elem);
      return false;
    }
    return false;
  ...
  _addSysTimedHandler(period, handler) {
    const thand = new _timedHandler.default(period, handler);
    thand.user = false;
    this.addTimeds.push(thand);
    return thand;
  }
  ...
  _addSysHandler(handler, ns, name, type, id) {
    const hand = new _handler.default(handler, ns, name, type, id);
    hand.user = false;
    this.addHandlers.push(hand);
    return hand;
  }
  ...
  _onDisconnectTimeout() {
    _core.default.debug('_onDisconnectTimeout was called');
    this._changeConnectStatus(_constants.Status.CONNTIMEOUT, null);
