...
class Handler {
  ...
  constructor(handler, ns, name, type, id, from, options) {
    ...
    if (this.options.matchBare) {
      _core.default.warn('The "matchBare" option is deprecated, use "matchBareFromJid" instead.');
      this.options.matchBareFromJid = this.options.matchBare;
      delete this.options.matchBare;
    }
    if (this.options.matchBareFromJid) {
      this.from = from ? (0, _utils.getBareJidFromJid)(from) : null;
    } else {
      this.from = from;
    }
  ...
  namespaceMatch(elem) {
    ...
    if (!this.ns) {
      ...
      (0, _utils.forEachChild)(elem, null, elem => {
  ...
  isMatch(elem) {
    ...
    if (this.options.matchBareFromJid) {
      from = (0, _utils.getBareJidFromJid)(from);
    }
    const elem_type = elem.getAttribute('type');
    if (this.namespaceMatch(elem) && (!this.name || _core.default.isTagEqual(elem, this.name)) && (!this.type || (Array.isArray(this.type) ? this.type.indexOf(elem_type) !== -1 : elem_type === this.type)) && (!this.id || elem.getAttribute('id') === this.id) && (!this.from || from === this.from)) {
      return true;
    }
    return false;
  }
